package club.gt.elasticsearch.service.aggregation;
...
public class AggrMetricService {
    ...
    public Object aggregationCount() {
        ...
        try {
            // 设置聚合条件
            TermsAggregationBuilder aggr = AggregationBuilders.terms("employee_count").field("salary");
            // 查询源构建器
            SearchSourceBuilder searchSourceBuilder = new SearchSourceBuilder();
            searchSourceBuilder.aggregation(aggr);
            searchSourceBuilder.size(0);
            ...
            if (RestStatus.OK.equals(response.status()) || aggregations != null) {
                // 转换为 ValueCount 对象
                Terms terms = aggregations.get("employee_count");
